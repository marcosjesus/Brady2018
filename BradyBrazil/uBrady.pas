...
uses uUtils, uRelatorioPreco, uRelatorioPartNumber, uCadastroSite, AsyncCalls, uCadastroCelula, uCadastroMaquina,
  ...
  uRelatorioGrossMargin, uConsultaNotaFiscal, uCadastroInovarCliente,
  ...
  uImportarIQFZMP05, uUploadForecast, uRelatorioAnaliseIQF, uUploadBOM,
  uUploadCustosEstoque, uUploadRouting, uUploadTaxasHoras, uCalcGM, uCadastroProdutoSeton,
  uUploadProdutoSeton, uRelatorioRecuperacaoContas, uRelatorioGestaoMercados;
...
begin
  ...
  if not Assigned(Fr_UploadTaxasHoras) then
    Fr_UploadTaxasHoras := TFr_UploadTaxasHoras.Create(Self);

  Fr_UploadTaxasHoras.Visible := True;
  Fr_UploadTaxasHoras.Update;
...
procedure TFr_Brady.dxBarLargeButtonSIOPRelatoriosGrossMarginClick(
  ...
  begin
    ...
    Fr_RelatorioGrossMargin.AbrirDataset;

  end;

begin

  if not Assigned(Fr_RelatorioGrossMargin) then
    Fr_RelatorioGrossMargin := TFr_RelatorioGrossMargin.Create(Self);

  Fr_RelatorioGrossMargin.Visible := True;
  Fr_RelatorioGrossMargin.BringToFront;
  Fr_RelatorioGrossMargin.Update;
...
begin
  ...
  try
    ...
    dxRibbonStatusBar.Panels[1].Text := 'Usuário: ' + WUserName.Trim;
    dxRibbonStatusBar.Panels[3].Text := 'Versão: ' + uUtils.FileVersion;
    dxRibbonStatusBar.Panels[5].Text := 'Máquina: ' + ComputerName;
    ...
    try
      ...
      try
        ...
        if FDQueryTSIS_Usuario.Eof and FDQueryTSIS_Usuario.Bof then
          raise Exception.Create('Usuário ("' + WUserName.Trim.ToUpper + '") não existe no sistema.');

        if FDQueryTSIS_UsuarioTSIS_USUATI.AsString.Equals('N') then
          raise Exception.Create('Usuário ("' + WUserName.Trim.ToUpper + '") não esta ativo no sistema.');
    ...
    begin
      ...
      Application.MessageBox( PWideChar(E.Message), 'Não foi possivel acessar o sistema', MB_ICONERROR+MB_OK );
