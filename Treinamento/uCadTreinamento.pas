...
begin
  ...
  varAprovado := '';
  Var_OperacaoItem := 0;
  If Mens_MensConf(pchar('Confirma Exclusão do Participante ' + sqlParticipanteFUN_NOME.AsString + ' ?')) <> mrOk then
    Exit;
...
begin
   ...
   Try
     ...
     begin
        ...
        begin
            Mens_MensInf('Participante já esta inscrito neste Treinamento!');
            LimpaTelaItens;
        end;
     end
     else
...
begin
   ...
   If ( Test_IsEmptyStr(EditBuscaTreinamento.Text) ) Then
	 Begin
			 Mens_MensInf('É necessário informar o Codigo do Treinamento ') ;
   ...
   If ( Test_IsEmptyStr(EditBuscaFornecedor.Text) ) Then
	 Begin
			 Mens_MensInf('É necessário informar o Nome do Fornecedor ') ;
   ...
   If ( Test_IsEmptyStr(ediCargaHoraria.Text) ) Then
	 Begin
			 Mens_MensInf('É necessário informar a Carga Horária ') ;
   ...
   begin
			 Mens_MensInf('Tipo do Treinamento não Informado! ') ;
   ...
   begin
			 Mens_MensInf('Periodicidade não Informado! ') ;
			 cmbPeriodo.SetFocus ;
			 Exit ;
   end
   else
    ...
    Begin
        Mens_MensInf('Data Inicial não pode ser nula') ;
        EdiDatIni.SetFocus ;
    ...
    Begin
        Mens_MensInf('Data Término não pode ser nula') ;
        EdiDatFim.SetFocus ;
...
begin
  inherited;
  If  (Mens_MensConf(  'Confirma Exclusão do Log do Treinamento Nrº ' + EditBuscaTreinamento.bs_KeyValue + ' ?') = mrOk ) Then
  begin
    qryAux.Close;
    qryAux.SQL.Clear;
...
begin
  inherited;
  If ( Mens_MensConf( 'Confirma Exclusão de Todos os Logs ?') = mrOk ) Then
  begin
    qryAux.Close;
    qryAux.SQL.Clear;
           ...
           for X := 1 to 3 do
            ...
            begin
                ...
                begin
                  ...
                  AsVariant := 'XXXXXXX... - Até 50 Posições';   // Nome da empresa
                ...
                begin
                  ...
                  AsVariant :=  'XXXXXXX... - Até 6 Posições';   // Matricula
                ...
                begin
                  ...
                  AsVariant := 'XXXXXXX... - Até 100 Posições';  // Nome do Participante
                ...
                begin
                  ...
                  AsVariant := '99/99/9999';  // Data de Admissão
                ...
                begin
                  ...
                  AsVariant := 'XXXXXXX... - Até 100 Posições';  // Nome do Cargo
                ...
                begin
                  ...
                  AsVariant := 'XXXXXXX... - Até 20 Posições';  // Codigo do Centro de Custo
                ...
                begin
                  ...
                  AsVariant := 'XXXXXXX... - Até 100 Posições';  // Descrição do Centro de Custo
...
begin
  ...
  Try
    ...
    Except
        On E:Exception do
         begin
           Mens_MensInf( 'Falha ao Inserir Log de Erro de Importação: ' + E.Message );
         end;
    End;
...
var
  ...
  varDataNascimento , varDataAdmissao  : TDateTime;
...
begin
  ...
  try
    ...
    try
      ...
      try
        ...
        begin
          ...
          try
            ...
            begin
             varCNPJ_EMPRESA := '';
             SalvarLogImport('CNPJ', 'CNPJ Não Informado');
            end
            ...
            if varMATRICULA = '' then
              SalvarLogImport('MATRICULA', 'Numero da Matricula não informado.');


            if not Assigned(dxSpreadSheet.ActiveSheetAsTable.Rows[X].Cells[1]) then
            begin
              varNOME_EMPRESA := '';
              SalvarLogImport('Nome Empresa', 'Nome da Empresa não informado');
            end
            ...
            begin
               SalvarLogImport('Nome Participante', 'Nome da Participante não informado');
               Continue;
            end;
            ...
            begin
              varSEXO := '';
              SalvarLogImport('Sexo', 'Sexo não informado');
            end
            ...
            begin
              varCARGO := '';
              SalvarLogImport('Cargo', 'Cargo não informado');
            end
            ...
            begin
              ...
              SalvarLogImport('Centro de Custo', 'Centro de Custo não informado');
            end
            else varCODIGO_CENTROCUSTO      :=  dxSpreadSheet.ActiveSheetAsTable.Rows[X].Cells[8].AsString;

            if not Assigned(dxSpreadSheet.ActiveSheetAsTable.Rows[X].Cells[9]) then
            begin
              varDESCRICAO_CENTROCUSTO := '';
              SalvarLogImport('Descrição Centro Custo', 'Dscrição do Centro de Custo não informado');
            end
            else varDESCRICAO_CENTROCUSTO      :=  dxSpreadSheet.ActiveSheetAsTable.Rows[X].Cells[9].AsString;
      ...
      if bLog then
      begin
        Application.MessageBox( 'Visualizar Eventos de Log na Importação', 'Visualizar', MB_ICONINFORMATION );
        AbrirTabelaLog;
