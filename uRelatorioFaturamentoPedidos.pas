...
uses
  ...
  dxSkinWhiteprint, dxSkinXmas2008Blue, cxHyperLinkEdit;
...
type
  TFr_RelatorioFaturamentoPedidos = class(TForm)
    ...
    cxTableViewFaturamentoPedidos00TSOP_PRIMARYCATALOG: TcxGridDBColumn;
    FDQueryVSOP_OrderBilling00TSOP_CUSTOMERPURCHASE: TStringField;
    cxTableViewFaturamentoPedidos00TSOP_CUSTOMERPURCHASE: TcxGridDBColumn;
    FDQueryVSOP_OrderBilling00TSOP_LINKSALESFORCE: TStringField;
    cxTableViewFaturamentoPedidos00TSOP_LINKSALESFORCE: TcxGridDBColumn;
    ...
    procedure cxTableViewFaturamentoPedidos00CellClick(
      Sender: TcxCustomGridTableView;
      ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
      AShift: TShiftState; var AHandled: Boolean);
    procedure cxTableViewFaturamentoPedidos00TSOP_ORDBILCLICODStylesGetContentStyle(
      Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
      AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
    procedure cxTableViewFaturamentoPedidos00MouseMove(Sender: TObject;
...
uses uBrady, uUtils, uUtilsOwner, uDados,ShellAPI;
...
begin
  ...
  try
    ...
    Try
        Mensagem( 'Obtendo dados (01 - Captado para o mês corrente)...' );
        ...
        Mensagem( 'Obtendo dados (06 - Faturamento do mês)...' );
        ...
        Mensagem( 'Obtendo dados (09 - Faturamento (backlog futuro) * antecipação)...' );
...
procedure TFr_RelatorioFaturamentoPedidos.cxTableViewFaturamentoPedidos00CellClick(
...
begin
  ...
  if (ACellViewInfo.Item = cxTableViewFaturamentoPedidos00TSOP_ORDBILCLICOD) and (AButton = mbLeft) then
  begin
...
procedure TFr_RelatorioFaturamentoPedidos.cxTableViewFaturamentoPedidos00MouseMove(
...
procedure TFr_RelatorioFaturamentoPedidos.cxTableViewFaturamentoPedidos00TSOP_ORDBILCLICODStylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
//var
...
begin
 { try
    AColumn := Sender.FindItemByName('cxTableViewFaturamentoPedidos00TSOP_LINKSALESFORCE');
    Status  := ARecord.Values[AColumn.Index];

    if (Status = Null)  then
    begin
      AStyle := cxStyleSemLink;
      exit;
    end;
   Except
...
const
  cArrayOfMonth: array[1..12] of String = ('Agosto','Setembro','Outubro','Novembro','Dezembro','Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho');
begin
...
procedure TFr_RelatorioFaturamentoPedidos.FormMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
